...
class Trainer:
    ...
    def train(
            ...
            checkpointing_frequency: int = 10000):
        """
        ...
        if self.is_main_process:
            with torch.no_grad():
                ...
                observations = batch.cuda()[:, :self.config["num_observations"]]
    ...
    def load_checkpoint(self, model: nn.Module, checkpoint_name: str = None):
        ...
        is_ddp_checkpoint = any(k.startswith("module.") for k in loaded_state["model"].keys())
        
        if is_ddp_checkpoint:
            state = {k.replace("module.", "", 1): v for k, v in loaded_state["model"].items()}
        else:
